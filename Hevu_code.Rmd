---
title: "H. vulnerata ms R scripts"
date: "2023-12-27"
...
```{r He and Ho}
gen <- adegenet::read.genepop("populations.snps.gen")
...
```{r PCA}
hevu <- read.table("populations.structure", sep = "\t", header = TRUE)
View(hevu)
...
dat <- list.files(pattern = "csv") # import all csv files with decimal degrees between populations to create vectors between each one. 
...
# model1 --> distance
# model2 --> habitat only
# model3 --> bioclim subset

data <- read.csv("data.csv") # fst values between each population, along with the environmental data and geographic distance.
head(data)

str(data) 

Zl <- lapply(c("pop1","pop2"), function(nm) 
  Matrix:::fac2sparse(hevu_data[[nm]], "d", drop=FALSE))
ZZ <- Reduce("+", Zl[-1], Zl[[1]])

df.clim <- with(data, data.frame(env)) # must omit the non-bioclimatic variables

usdm::vif(df.clim)

df.hab <- with(hevu_data, data.frame(env))
...
# model1 --> distance
# model2 --> grinnellian niche 
# model3 --> bioclim niche

mod1 <- MLPE(log.fst ~ LENGTH + (1|pop1), data)

mod2 <- MLPE(log.fst ~ bio2 + bio8 + bio9 + bio15 +
               direction + stream + dem + tcc +
               (1|pop1), data)

mod3 <- MLPE(log.fst ~ bio2 + bio8 + bio9 + 
...
mod1noREML <- MLPEnoREML(log.fst ~ LENGTH + (1|pop1), data)

mod2noREML <- MLPEnoREML(log.fst ~ bio2 + bio8 + bio9 + bio15 +
               direction + stream + dem + tcc + (1|pop1), data)

mod3noREML <- MLPEnoREML(log.fst ~ bio2 + bio8 + bio9 + 
               bio17 + bio18 + bio19 + (1|pop1), data)


Models <- list(Distance=mod1noREML, Grinnellian = mod2noREML, Bioclim = mod3noREML)
...
ModelsREML <- list(Grinnellian=mod2, Bioclim=mod3)

# 95% CI for these estimates
confint(ModelsREML$Distance, level = 0.95, method = "Wald")
